...
def concat_images(images, output_path):
    ...
    images_to_concat = [img for img in images if img.endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif'))] #ligne rajoutée à cause d'une erreur sinon
    opened_images = [Image.open(img).convert('RGB') for img in images_to_concat]  # Convertir chaque image en RGB
...
def automate_image_combination():
    ...
    for combination in selected_combinations:
        ...
        part1 = "a01"                 # Fixe ou généré d'une certaine manière
        part2 = "000u"                # Fixe ou basé sur une logique
        part3 = "00"                  # Fixe ou dynamique selon le contexte
        part4 = f"{counter:02d}"      # Numéro dynamique avec 2 chiffres de padding

        output_file_name = f"{part1}-{part2}-{part3}-{part4}.png"   
